...
# tags: ["Type Theory", "Lambda Calculi", "Haskell", "Type Checking"]
categories:
  - posts
date: "2019-10-27"
draft: false
...
Bidirectional typechecking is a technique for turning a system of typing judgments into algorithm for typechecking. Beyond the relative ease of implementation this technique can encompass a vast array of type system features (higher-rank polymorphism, intersection types, dependent types etc.).
Usually in type theory we write inference rules to express some forms of knowledge about our system. For instance let us give we might express the following equality judgments relating addition of natural numbers:
...
\def\m{\mathrm{m}}
\def\p{\mathrm{p}}
\def\ty#1#2{\mathrm{#1} \; \colon \; \mathrm{#2}}
\def\con#1#2{\mathrm{#1} \; ; \; \mathrm{#2}}
\frac{\Gamma  \vdash \ty{n}{\nat}}{\Gamma \vdash 0 + \n \eqNat n}
  $$

$$
\frac{\Gamma  \vdash \n + \m \eqNat \p}{\Gamma \vdash \suc \n + \m \eqNat \suc \p}
  $$

We can read these judgments as follows:
  
  -  Suppose given assumptions $\Gamma$ we know that $\n$ is a $\nat$. We then know that $0 + \n$ is equal (as $\mathrm{nats}$) to $\n$.

  - Similarly if (in context $\Gamma$) we know an equality of $\mathrm{nats}$, $\n + \m \equiv \p \; \nat$, then we know $\suc \n + \m = \suc \p \; \nat$.

Import for us in these inference rules is that they do not (yet) constitute an algorithm for addition. It is reasonable for us to read the left hand side as inputs and the right hand side as outputs and we would then be tempted to write a definition of the form:
```haskell
add :: Nat -> Nat -> Nat
add Zero     n = n
add (Succ n) m = Suc (add n m)
```
In order for this to be justified as capturing the judgment we would need to prove that for any $\m$ and $\n$ there is a unique $p$ such that $\m + \n \eqNat \p$. This is straightforward by induction on the rules. In order to express this fact we can assign a mode to the judgment





  Let us start with re-analysing the usual typing judgment:
  
$$ \Gamma \vdash \mathrm{a} : \mathrm{A} $$

We read this as saying that given some context $ \Gamma $ of type declarations, term $\mathrm{a}$ has type $\mathrm{A}$. Note that this judgment is completely _declarative_ - that is to say, when we try to read the judgment algorithmically we do not know the _mode_ of each variable i.e. whether it is intended as input or output.
As an example of some different modes:

  - If we are given all of $\Gamma$, $\mathrm{a}$ and $\mathrm{A}$ as inputs then we are
    are performing _type checking_, we are tasked with checking whether it is indeed the
     case that this judgment holds.

  - If we are given only $\Gamma$ and $\mathrm{a}$ as inputs then this is _type synthesis_.
    In this case we have to find a type for which the typing judgment holds.

  - If instead we are given $\Gamma$ and $\mathrm{A}$ then this is _program synthesis_.

For this article we will be most interested in the first two and so we will split our typing
judgment into two judgements, one a checking judgment and one synthesis. We write these as follows:

$$ \Gamma \vdash \mathrm{a} \color{red}{\Rightarrow} \mathrm{A} \quad  \mathrm{[type} \; \mathrm{synthesis]}$$

$$ \Gamma \vdash \mathrm{a} \color{cyan}{\Leftarrow} \mathrm{A} \quad  \mathrm{[type} \; \mathrm{checking]}$$ 
